...
Whenever changes are made to `git-ingest`, the following documentation synchronization checks must be satisfied:

1. **CLI Flags & Arguments**:
   - Any new or modified CLI flag in `src/index.ts` must be documented in `README.md` and `skills/git-ingest/SKILL.md`.
...
4. **AI Agent Skill**:
   - Workflows, tool descriptions, or default paths in `skills/git-ingest/SKILL.md` must match the active implementation.
...
- The global skill file `skills/git-ingest/SKILL.md` is valid YAML/Markdown.
